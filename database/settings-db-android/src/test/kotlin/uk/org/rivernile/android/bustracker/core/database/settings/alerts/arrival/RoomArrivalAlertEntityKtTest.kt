...
/**
 * Tests for `RoomArrivalAlertEntity.kt`.
 *
 * @author Niall Scott
 */
class RoomArrivalAlertEntityKtTest {

    @Test
    fun toArrivalAlertMapsToRoomArrivalAlertEntity() {
        val result = FakeArrivalAlert(
            ...
            services = emptySet()
        ).toArrivalAlertEntity()

        assertEquals(
            RoomArrivalAlertEntity(
                id = 0,
                timeAdded = Instant.fromEpochMilliseconds(123L),
                stopIdentifier = "123456".toNaptanStopIdentifier(),
                timeTriggerMinutes = 5
