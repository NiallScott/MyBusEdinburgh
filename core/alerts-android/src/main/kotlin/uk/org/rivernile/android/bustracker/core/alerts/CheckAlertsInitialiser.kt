...
/**
 * An app [Initializer] which checks if there are alerts set by the user and if they are, then
 * begins monitoring the alerts.
 *
 * @author Niall Scott
 */
@Suppress("unused")
class CheckAlertsInitialiser : Initializer<Unit> {

    override fun create(context: Context) {
        val hiltEntryPoint = EarlyEntryPoints.get(
            context,
            CheckAlertsInitialiserEntryPoint::class.java)

        hiltEntryPoint.applicationCoroutineScope.launch(hiltEntryPoint.defaultDispatcher) {
            hiltEntryPoint.alertsRepository.ensureTasksRunningIfAlertsExists()
        }
    }

    override fun dependencies(): List<Class<out Initializer<*>>> =
        listOf(NotificationChannelsInitialiser::class.java)

    @EarlyEntryPoint
    @InstallIn(SingletonComponent::class)
    internal interface CheckAlertsInitialiserEntryPoint {
