...
/**
 * A fake [AlertNotificationDispatcher] for testing.
 *
 * @author Niall Scott
 */
class FakeAlertNotificationDispatcher(
    private val onDispatchTimeAlertNotification: (ArrivalAlert, List<Service>) -> Unit =
        { _, _ -> throw NotImplementedError() },
    private val onDispatchProximityAlertNotification: (ProximityAlert) -> Unit =
    ...
    override suspend fun dispatchTimeAlertNotification(
        ...
        onDispatchTimeAlertNotification(arrivalAlert, qualifyingServices)
    }

    override suspend fun dispatchProximityAlertNotification(proximityAlert: ProximityAlert) {
        onDispatchProximityAlertNotification(proximityAlert)
    }
}