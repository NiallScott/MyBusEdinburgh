...
class RealStateTest {
    ...
    @Test
    fun searchTermIsNullByDefault() = runTest {
        val state = createState()

        state.searchTermFlow.test {
            assertNull(awaitItem())
            ensureAllEventsConsumed()
        }
        assertNull(state.action)
    ...
    @Test
    fun searchTermIsMutatedToTheCorrectValue() = runTest {
        val state = createState()

        state.searchTermFlow.test {
            assertNull(awaitItem())
            state.searchTerm = "search term"
            assertEquals("search term", awaitItem())
            assertEquals("search term", state.searchTerm)
    ...
    @Test
    fun searchTermIsInstantiatedToValueOfSavedState() = runTest {
        val state = createState(
            savedState = SavedStateHandle(
                initialState = mapOf(
                    STATE_SEARCH_TERM to "search term"
                )
            )
        )
