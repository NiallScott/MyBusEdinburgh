...
public class BusStopMapActivity extends AppCompatActivity implements BusStopMapFragment.Callbacks,
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        // Only add the fragment if there was no previous instance of this Activity, otherwise this
        // fragment will appear multiple times.
        if (savedInstanceState == null) {
            ...
            if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
                ...
                f = BusStopMapFragment.newInstance(intent.getStringExtra(ARG_STOPCODE));
            } else if (intent.hasExtra(ARG_LATITUDE) && intent.hasExtra(ARG_LONGITUDE)) {
    ...
    public void onNewIntent(final Intent newIntent) {
        ...
        final BusStopMapFragment f = (BusStopMapFragment) getSupportFragmentManager()
        ...
        } else if (newIntent.hasExtra(ARG_LATITUDE) && newIntent.hasExtra(ARG_LONGITUDE)) {
            f.moveCameraToLocation(new LatLng(newIntent.getDoubleExtra(ARG_LATITUDE, 0),
    ...
    @Override
    public void onShowServicesChooser(final String[] services, final String[] selectedServices,
                                      final String title) {
        ServicesChooserDialogFragment.newInstance(services, selectedServices, title)
    ...
    public void onMapTypeChosen(final int mapType) {
        try {
            final MapTypeChooserDialogFragment.Callbacks child =
                    (MapTypeChooserDialogFragment.Callbacks) getSupportFragmentManager()
                            .findFragmentById(R.id.fragmentContainer);
